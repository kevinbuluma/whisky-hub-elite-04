...
function Cart() {
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-2xl mx-auto px-6 py-32 text-center">
          ...
          <p className="text-foreground/60 mb-8">
            Find your next favorite bottle in our collection.
          </p>
          <Link
            to="/shop"
            className="inline-block bg-amber text-black font-bold py-4 px-8 rounded-full text-sm uppercase tracking-widest hover:bg-gold transition"
          >
            Browse Spirits
          </Link>
        </div>
  ...
  const checkout = () => {
    const summary = items
      .map((i) => `• ${i.qty}× ${i.product.name} — ${fmtKES(i.product.price * i.qty)}`)
      .join("\n");
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-4">
                ...
                <div className="flex flex-col items-end justify-between">
                  ...
                  <div className="flex items-center gap-2 bg-background border border-border rounded-full">
                    <button
                      ...
                      className="size-8 grid place-items-center hover:text-amber"
                    >
            ...
            <button
              onClick={clear}
              className="text-xs uppercase tracking-widest text-foreground/40 hover:text-destructive"
            >
              Clear cart
            </button>
          </div>
          ...
          <aside className="bg-surface border border-border rounded-2xl p-6 h-fit lg:sticky lg:top-24 space-y-5">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-mono">{fmtKES(total)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-foreground/60">Delivery (Rongai)</span>
                <span className="font-mono">{delivery === 0 ? "FREE" : fmtKES(delivery)}</span>
              </div>
              <div className="flex justify-between text-lg font-bold pt-3 border-t border-border">
                <span>Total</span>
                <span className="font-mono text-amber">{fmtKES(grand)}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              {[
                ...
                <button
                  key={opt.v}
                  onClick={() => setPay(opt.v as "mpesa" | "card" | "cod")}
